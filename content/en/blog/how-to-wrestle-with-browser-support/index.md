...
The essence of both questions lies in environment detection. Web is not a *binary* platform like iOS or Android, but a huge set of configurations. In principle, it is not possible to create a unified experience — the application must be **"responsive"** from the perspective of *UX*, too. One way to detect an environment is to ask for its **name** and **version**, and make a decision based on the answer. Typically, we would look at the HTTP header `User Agent`[^1]. The header, though, offers no guarantee and is often false, which makes this method useless. So, rather than ask for an environment's name, it's better to query about its **properties**[^2]. If the response meets our requirements, we can then decide with a fair amount of confidence.

## Real users

Finally, it's time to set the boundary, which is a small set of **properties** we deem necessary. Browsers that don’t "cut the mustard" will receive a basic — but still useful — version without JavaScript. Now is a good idea to use some form of **web analytics**. It will allow you to evaluate real users' environment (their browser, device and operating system) and decide not on intuition but data. If half of the users use Internet Explorer 8, it makes sense to choose a different approach than if there is just a handful of those poor souls.
...
After we decide on our target group, we'll add a simple condition into our application (let's call it `app.js`) to check the chosen properties. In case of failure, we will immediately stop execution of the script[^3]. If, however, the browser passes the test, we let the application do its job, and also add a `.js` class to the HTML document. Thanks to this last detail, we can deduce whether the application is running even outside the context of the script (like in stylesheets). That will be helpful when designing components.

For example, let's make a component with the class `.accordion` and the following structure: a heading, which is also a `button`, and some content under it, which is hidden by default and shows only after clicking the button[^4]. A common method is to hide the content, perhaps with `display: none`, and reveal it with the help of JavaScript by adding the class `.is-active` on the component[^5]. However, if the script fails to load for any reason, the content is lost forever (or until we fix the problem). That's why we use the `.js` class and rewrite the component's styles so that the logic is reversed: the content is visible and hides only if we are certain the script is running[^6]. It's a trivial change with a major impact, truly in the spirit of the [progressive enhancement](/blog/progressive-enhancement/).

## *Polyfilling*

When we return back to `app.js`, we encounter another problem. 
Thanks to the `return false` in our application, we let the browsers that don’t pass the test save their face. But they still have to download the whole script, which can amount to hundreds of kilobytes, perhaps even a few megabytes. There’s a solution that consists of a little scripts — let’s call it `scout.js` — with a single purpose: to test the environment and, if positive, load the rest of the application[^7].

Of course, there are some obstacles. We can deduce, based on the set of properties in the example, that the application is supposed to run in Internet Explorer 10 and 11. The example, however, makes use of `fetch`, which is an API for making AJAX calls based on Promise* and which, sadly, is missing in these browsers. The script would therefore fail spectacularly. 
