...
I přes svou malou velikost ikony představují pokud ne zásadní, tak alespoň zajímavý problém. Jeden přístup střídá další — každý o trochu lepší než ten předchozí, každý nějakým způsobem nedokonalý.

<!--more-->

Poslední systém, kterému se delší dobu daří udržet navrcholu, je takzvaný SVG sprite, který vznikne tak, že jednotlivé ikony (respektive jejich SVG) vložíme do elementů `symbol` v rámci jednoho SVG souboru. Důvodem pro použití symbolů je to, že se nevykreslí v místě definice, ale až tehdy, kdy je skutečně použijeme. Pokud tento sprite nechceme vytvářet ručně, lze použít jedno z mnoha dostupných řešení, jako třeba grunt-svgstore nebo gulp-svgsprite.
